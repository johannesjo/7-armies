...
describe('isInRange with elevation', () => {
  it('uses base range without elevation', () => {
    const attacker = createUnit('a1', 'archer', 'blue', { x: 0, y: 0 });
    // archer: range=220, radius=8. target radius=8. Distance must exceed range + both radii = 236
    const target = createUnit('t1', 'archer', 'red', { x: 237, y: 0 });
    expect(isInRange(attacker, target)).toBe(false);
    expect(isInRange(attacker, target, [])).toBe(false);
  });

  it('applies elevation bonus when archer is on hill', () => {
    const zone: ElevationZone = { x: 0, y: 0, w: 50, h: 50 };
    const attacker = createUnit('a1', 'archer', 'blue', { x: 25, y: 25 });
    // Place target just outside base range but within elevated range
    const baseMax = attacker.range + attacker.radius + 8; // 8 = target radius (archer)
    const elevatedMax = attacker.range * (1 + ELEVATION_RANGE_BONUS) + attacker.radius + 8;
    const dist = (baseMax + elevatedMax) / 2; // between base and elevated max
    const target = createUnit('t1', 'archer', 'red', { x: 25 + dist, y: 25 });

    expect(isInRange(attacker, target)).toBe(false);
    expect(isInRange(attacker, target, [zone])).toBe(true);
  });

  it('does not apply bonus when archer is off hill', () => {
    const zone: ElevationZone = { x: 500, y: 500, w: 50, h: 50 };
    const attacker = createUnit('a1', 'archer', 'blue', { x: 0, y: 0 });
    const target = createUnit('t1', 'archer', 'red', { x: attacker.range + attacker.radius + 8 + 5, y: 0 });
  ...
  it('applies stacking bonus with two overlapping zones', () => {
    ...
    const stackedMax = attacker.range * (1 + ELEVATION_RANGE_BONUS * 2) + attacker.radius + 8;
    const singleMax = attacker.range * (1 + ELEVATION_RANGE_BONUS) + attacker.radius + 8;
  ...
  it('melee units get no elevation range bonus', () => {
    const zone: ElevationZone = { x: 0, y: 0, w: 50, h: 50 };
    const swordsman = createUnit('sw1', 'swordsman', 'blue', { x: 25, y: 25 });
    // swordsman: range=25, radius=10, target radius=10. Max = 45
    const target = createUnit('t1', 'swordsman', 'red', { x: 25 + 46, y: 25 });

    // Should be out of range even on hill (no elevation bonus for melee)
    expect(isInRange(swordsman, target, [zone])).toBe(false);
  });
});
...
describe('tryFireProjectile with elevation', () => {
  it('produces projectile with elevated maxRange when on hill', () => {
    ...
    const target = createUnit('t1', 'swordsman', 'red', { x: 200, y: 100 });
